...
**Step 9b: Platform Doc Staleness Check** (T1520 — if `.gald3r/PLATFORM_STATUS.md` exists)

A lightweight read-only check that surfaces platforms whose official docs have not been re-crawled inside the configured freshness window. It NEVER writes, modifies, or creates any file — it only alerts. The full re-crawl is `@g-platform-scan` (which delegates to `custom_scripts/scan_platform_docs.ps1 -StaleOnly`).

- Read `.gald3r/PLATFORM_STATUS.md`. If the file does not exist, skip silently.
- For each table row, parse the `Last Doc Scan` column. Skip rows where the value is `never`, empty, or not a `YYYY-MM-DD` date (those are surfaced by the script itself, not by session start).
- Count rows where the parsed date is older than the `crawl_max_age_days` threshold (default `7`; configurable via `crawl_max_age_days:` in `.gald3r/config/AGENT_CONFIG.md`).
- If count > 0 → display: `📚 N platform(s) overdue for doc re-crawl — run @g-platform-scan`
- If count == 0 → silent (no output).

**Rules:**
- Reads exactly **one file** (`PLATFORM_STATUS.md`). No vault touch, no script run.
- Completes in under 100 ms — pure markdown-table scan.
- **Never writes, modifies, or creates** any file.
- Skips silently if `PLATFORM_STATUS.md` does not exist.
