...
```powershell
gald3r workspace member bootstrap `
    --member-path "<absolute_member_path>" `
    --member-id "<manifest_repo_id>" `
    [--controller-path "<absolute_controller_path>"] `   # optional; defaults to upward manifest discovery
...
```powershell
# Dry-run
gald3r workspace member remediate `
    --member-path "<absolute_member_path>"
...
```powershell
gald3r workspace member validate
```
...
```powershell
# Dry-run (default)
gald3r workspace member promote `
    --member-path "<absolute_member_path>" `
    [--member-id "<manifest_repo_id>"] `      # inferred from manifest/.identity when omitted
