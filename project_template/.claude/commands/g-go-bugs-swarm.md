...
2. **Housekeeping commit gate** — `gald3r housekeep -Mode preflight -Apply -Json`
...
1. **Pre-Reconciliation Clean Gate** — re-run `git status --short` at orchestration root
   - **Lock report (T1059)** — run `gald3r worktree lock-report -Json` and surface any multi-bucket path in the summary as a `WARN` (not a hard block) before applying patches.
...
4. **Post-write housekeeping gate**: `gald3r housekeep -Mode post-write -Apply -Json`
