...
2. Invoke the worktree helper to append the item:
   ```powershell
   .\.claude\skills\g-skl-git-commit\scripts\gald3r_worktree.ps1 -Action Queue -TaskId {id} -Role code -Owner {owner} -QueueText "<follow-up prompt>" -Json
   ```
...
```powershell
.\.claude\skills\g-skl-git-commit\scripts\gald3r_worktree.ps1 -Action Queue -TaskId {id} -Role code -Owner {owner} -Json
```
