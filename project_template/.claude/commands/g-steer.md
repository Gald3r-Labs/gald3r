...
2. Invoke the worktree helper to write `steer.md` (overwrite — latest steer wins):
   ```powershell
   .\.claude\skills\g-skl-git-commit\scripts\gald3r_worktree.py -Action Steer -TaskId {id} -Role code -Owner {owner} -SteerText "<steering prompt>" -Json
   ```
   Checkouts that ship a root `scripts/gald3r_worktree.py` copy (the gald3r source repo)
...
```powershell
.\.claude\skills\g-skl-git-commit\scripts\gald3r_worktree.py -Action Steer -TaskId {id} -Role code -Owner {owner} -Json
```
...
- Helper: `.claude/skills/g-skl-git-commit/scripts/gald3r_worktree.py` (`-Action Steer`)
