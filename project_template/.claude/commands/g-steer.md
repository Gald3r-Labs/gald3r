...
2. Invoke the worktree helper to write `steer.md` (overwrite — latest steer wins):
   ```powershell
   .\.claude\skills\g-skl-git-commit\scripts\gald3r_worktree.ps1 -Action Steer -TaskId {id} -Role code -Owner {owner} -SteerText "<steering prompt>" -Json
   ```
...
```powershell
.\.claude\skills\g-skl-git-commit\scripts\gald3r_worktree.ps1 -Action Steer -TaskId {id} -Role code -Owner {owner} -Json
```
