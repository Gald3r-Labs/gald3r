...
- `.gitignore`: `/.gald3r_sys` root-anchored entry added (root is a junction; canonical tracked under `claude_cursor_unified_template/.gald3r_sys`).
- PCAC → WPAC rename propagated across g-go* command suite (`.claude/commands/`, `.cursor/commands/`, canonical `claude_cursor_unified_template/.gald3r_sys/`).
