...
2. For each, show: `{task_id}` · proposed slug · one-line `description` · `proposed_date` · whether a
   same-named skill already exists in `.cursor/skills/`.
...
2. Write the polished SKILL.md to **`.cursor/skills/{slug}/SKILL.md`** (canonical source).
...
- This command only ever writes under `.gald3r/proposed_skills/` and (on PROMOTE) `.cursor/skills/`.
