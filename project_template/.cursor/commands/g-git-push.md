...
Run **`.cursor/skills/g-skl-git-commit/scripts/gald3r_push_gate.ps1`** before `git push` to distinguish **routine** pushes from **release** pushes. Complements `@g-git-sanity` / `g-hk-pre-commit.ps1` and shared `.cursor/skills/g-skl-git-commit/scripts/gald3r_git_sanity_common.ps1`.
...
./.cursor/skills/g-skl-git-commit/scripts/gald3r_push_gate.ps1

# Explicit release checks
./.cursor/skills/g-skl-git-commit/scripts/gald3r_push_gate.ps1 -Release

# CI / non-interactive release
$env:GALD3R_RELEASE_PUSH = "1"
./.cursor/skills/g-skl-git-commit/scripts/gald3r_push_gate.ps1 -NonInteractive

# Agent wiring check (always exit 0)
./.cursor/skills/g-skl-git-commit/scripts/gald3r_push_gate.ps1 -DryRun
./.cursor/skills/g-skl-git-commit/scripts/gald3r_push_gate.ps1 -Release -DryRun
```
...
./.cursor/skills/g-skl-git-commit/scripts/gald3r_push_gate.ps1 -Release -NonInteractive
```
...
Set `enabled: true` in `.gald3r/config/COMPLIANCE_GATE.md`. When enabled, `.cursor/skills/g-skl-git-commit/scripts/gald3r_push_gate.ps1` automatically adds `-ComplianceCheck` before the push proceeds.

### Gate Behavior

The compliance check runs `.cursor/skills/g-skl-compliance/scripts/run_compliance_scan.ps1 --gate-mode` and interprets exit codes:
...
If `.cursor/skills/g-skl-compliance/scripts/run_compliance_scan.ps1` is a stub (T906 not yet complete), the gate detects it and skips gracefully:
```
