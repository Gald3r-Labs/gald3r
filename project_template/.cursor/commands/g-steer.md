...
2. Invoke the worktree helper to write `steer.md` (overwrite — latest steer wins):
   ```powershell
   .\.cursor\skills\g-skl-git-commit\scripts\gald3r_worktree.py -Action Steer -TaskId {id} -Role code -Owner {owner} -SteerText "<steering prompt>" -Json
   ```
...
```powershell
.\.cursor\skills\g-skl-git-commit\scripts\gald3r_worktree.py -Action Steer -TaskId {id} -Role code -Owner {owner} -Json
```
