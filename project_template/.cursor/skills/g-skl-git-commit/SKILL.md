...
- **Path encoding**: Claude Code names each project folder by replacing every non-alphanumeric char in the absolute cwd with `-` (e.g. `<workspace>\gald3r_dev` → `G--gald3r-ecosystem-gald3r-dev`). The helper reproduces this to find the worktree's session folder.
...
Before `git push`, run **`.cursor/skills/g-skl-git-commit/scripts/gald3r_push_gate.ps1`** (or `@g-git-push`) so **routine** work is not blocked by release documentation rules, while **release** pushes enforce CHANGELOG/version discipline (`g-rl-26`, `g-rl-02`).
...
```powershell
./.cursor/skills/g-skl-git-commit/scripts/gald3r_push_gate.ps1                    # interactive mode select
./.cursor/skills/g-skl-git-commit/scripts/gald3r_push_gate.ps1 -Release          # release checks
$env:GALD3R_RELEASE_PUSH='1'; ./.cursor/skills/g-skl-git-commit/scripts/gald3r_push_gate.ps1 -NonInteractive
./.cursor/skills/g-skl-git-commit/scripts/gald3r_push_gate.ps1 -DryRun           # verify wiring; always exit 0
```
...
`.cursor/skills/g-skl-git-commit/scripts/gald3r_git_sanity_common.ps1` supplies secret patterns for **`g-hk-pre-commit.ps1`**; push gate lives in **`.cursor/skills/g-skl-git-commit/scripts/gald3r_push_gate.ps1`**.
---
