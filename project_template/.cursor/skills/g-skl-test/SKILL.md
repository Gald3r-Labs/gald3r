...
L1/L2/L3 are **code-test** levels (they run test files via `scripts/tests/run_l1_tests.py`,
...
Distinct from L1/L2/L3 code tests: this runs `scripts/gald3r_system_test.py`, which
...
uv run python scripts/gald3r_system_test.py
...
uv run python scripts/gald3r_system_test.py -Json

# Subset of systems
uv run python scripts/gald3r_system_test.py -Systems "task,bug,hooks,git_hooks,schema"
```
...
- **CI / pre-release**: wire `python scripts/gald3r_system_test.py -FailBelow <N>` as a gate (see FUNCTIONAL above)
