...
param(
    # ΓöÇΓöÇ Installer mode params ΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇ
    ...
    # ΓöÇΓöÇ Session mode params (mirrors old setup_dev_env.ps1) ΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇ
...
# ΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇ
# Per-platform structure derived from each platform's PLATFORM_SPEC.md ┬º1 "Folder
...
# BEGIN_PLATFORM_CATS - AUTO-GENERATED by generate_platform_cats.ps1 - do not edit manually
$allPlatforms = [ordered]@{
    hermes      = @{ prefix = ""; scaffoldPrefix = ".hermes"; cats = @(); rootFiles = @("AGENTS.md","HERMES.md"); label = "Hermes (Nous Research)" }
    qoder       = @{ prefix = ".qoder"; cats = @("rules"); rootFiles = @(); label = "Qoder (Alibaba)" }
    codebuddy   = @{ prefix = ".codebuddy"; cats = @("skills"); rootFiles = @("AGENTS.md"); label = "CodeBuddy (Tencent)" }
    warp        = @{ prefix = ""; scaffoldPrefix = ".warp"; cats = @(); rootFiles = @("AGENTS.md","WARP.md"); label = "Warp Terminal" }
    kimi        = @{ prefix = ".kimi"; cats = @(); rootFiles = @("AGENTS.md"); label = "Kimi Code (Moonshot AI)" }
    roo         = @{ prefix = ".roo"; cats = @("commands","rules"); rootFiles = @(".roomodes",".roorules",".roorules-architect"); label = "Roo Code" }
    junie       = @{ prefix = ".junie"; cats = @("mcp"); rootFiles = @(); label = "JetBrains Junie" }
    cline       = @{ prefix = ".cline"; cats = @(); rootFiles = @(".clinerules","memory-bank"); label = "Cline" }
    kilo_code   = @{ prefix = ".kilo"; cats = @("agents","rules"); rootFiles = @(); label = "Kilo Code" }
    vibe        = @{ prefix = ""; cats = @(); rootFiles = @(); label = "vibe" }
    kiro        = @{ prefix = ".kiro"; cats = @("hooks","steering"); rootFiles = @(); label = "Kiro" }
    codex       = @{ prefix = ".codex"; cats = @("skills"); rootFiles = @(); label = "OpenAI Codex CLI" }
    cursor      = @{ prefix = ".cursor"; cats = @("agents","commands","hooks","rules","skills"); rootFiles = @(); label = "Cursor IDE" }
    claude      = @{ prefix = ".claude"; cats = @("agents","commands","rules","skills"); rootFiles = @(); label = "Claude Code" }
    ...
    antigravity = @{ prefix = ".agents"; cats = @("skills"); rootFiles = @(".antigravity/mcp.json","AGENTS.md"); label = "Antigravity (agy)" }
    ...
    astrbot     = @{ prefix = ""; scaffoldPrefix = ".astrbot"; cats = @(); rootFiles = @("AGENTS.md"); label = "AstrBot" }
    deepcode    = @{ prefix = ".deepcode"; cats = @("skills"); rootFiles = @(); label = "Deep Code (DeepSeek)" }
    replit      = @{ prefix = ".replit-gald3r"; cats = @(); rootFiles = @(".replit","replit.md","replit.nix"); label = "Replit Agent" }
    subq        = @{ prefix = ".subq"; cats = @(); rootFiles = @(); label = "SubQ" }
    qwen        = @{ prefix = ".qwen"; cats = @("commands"); rootFiles = @("QWEN.md"); label = "Qwen Code" }
    kiro_cli    = @{ prefix = ".kiro-cli"; cats = @("hooks","steering"); rootFiles = @(); label = "Kiro CLI" }
    opencode    = @{ prefix = ".opencode"; cats = @("agents","commands","skills"); rootFiles = @(); label = "OpenCode (sst.dev)" }
    continue    = @{ prefix = ".continue"; cats = @("rules"); rootFiles = @(); label = "Continue" }
    aider       = @{ prefix = ".aider"; cats = @(); rootFiles = @(".aider.conf.yml",".aiderignore","CONVENTIONS.md"); label = "Aider" }
    openhands   = @{ prefix = ".openhands"; cats = @("microagents","skills"); rootFiles = @("AGENTS.md"); label = "OpenHands" }
    trae        = @{ prefix = ".trae"; cats = @("rules","skills"); rootFiles = @(); label = "TRAE (ByteDance)" }
    amp         = @{ prefix = ".agents"; cats = @("skills"); rootFiles = @("AGENTS.md"); label = "Amp Code (Sourcegraph)" }
    void        = @{ prefix = ""; scaffoldPrefix = ".void"; cats = @(); rootFiles = @(".cursorrules",".vscode/settings.json"); label = "Void Editor" }
    openclaw    = @{ prefix = ".openclaw"; cats = @(); rootFiles = @("openclaw_instructions.md","SOUL.md"); label = "OpenClaw" }
    windsurf    = @{ prefix = ".windsurf"; cats = @("rules","workflows"); rootFiles = @(".windsurfrules"); label = "Windsurf" }
    mistral     = @{ prefix = ".vibe"; cats = @("agents","skills"); rootFiles = @("AGENTS.md"); label = "Mistral Vibe" }
...
if (-not $isInstallerMode) {
    ...
    function Get-ActivePlatform {
        ...
        foreach ($key in $allPlatforms.Keys) {
            ...
            if ($prefix -and (Test-Path (Join-Path $ProjectRoot $prefix))) { $detected += $key }
        }
    ...
    foreach ($platKey in $targets) {
        ...
        $scaffoldPrefix = if ($plat['scaffoldPrefix']) { $plat['scaffoldPrefix'] } else { $plat.prefix }

        if ([string]::IsNullOrWhiteSpace($plat.prefix)) {
            $scaffoldSrc = Join-Path $sysRoot "platforms\$scaffoldPrefix"
        ...
        # Phase 1: scaffold from .gald3r_sys/platforms/{scaffoldPrefix}
        $scaffoldSrc = Join-Path $sysRoot "platforms\$scaffoldPrefix"
        ...
        # Rules (with extension translation) - only when platform declares rules cat
        ...
        # into the prefix dir ΓÇö they only land at project root via this explicit copy.
        $rootPayloadSrc = Join-Path $scaffoldSrc "_rootpayload"
        if ($plat.rootFiles) {
            foreach ($rf in $plat.rootFiles) {
                $rfSrc = if (Test-Path (Join-Path $rootPayloadSrc $rf)) { Join-Path $rootPayloadSrc $rf } else { Join-Path $scaffoldSrc $rf }
...
function Write-Banner {
    Write-Host ""
    Write-Host "  ΓòöΓòÉΓòÉΓòÉΓòÉΓòÉΓòÉΓòÉΓòÉΓòÉΓòÉΓòÉΓòÉΓòÉΓòÉΓòÉΓòÉΓòÉΓòÉΓòÉΓòÉΓòÉΓòÉΓòÉΓòÉΓòÉΓòÉΓòÉΓòÉΓòÉΓòÉΓòÉΓòÉΓòÉΓòÉΓòÉΓòÉΓòÉΓòÉΓòÉΓòÉΓòÉΓòÉΓòÉΓòÉΓòÉΓòÉΓòù" -ForegroundColor Cyan
    Write-Host "  Γòæ        gald3r Project Installer v1.4         Γòæ" -ForegroundColor Cyan
    Write-Host "  Γòæ   AI Development System for Modern IDEs      Γòæ" -ForegroundColor Cyan
    Write-Host "  ΓòÜΓòÉΓòÉΓòÉΓòÉΓòÉΓòÉΓòÉΓòÉΓòÉΓòÉΓòÉΓòÉΓòÉΓòÉΓòÉΓòÉΓòÉΓòÉΓòÉΓòÉΓòÉΓòÉΓòÉΓòÉΓòÉΓòÉΓòÉΓòÉΓòÉΓòÉΓòÉΓòÉΓòÉΓòÉΓòÉΓòÉΓòÉΓòÉΓòÉΓòÉΓòÉΓòÉΓòÉΓòÉΓòÉΓòÉΓò¥" -ForegroundColor Cyan
    Write-Host ""
}
...
function Deploy-PlatformDirs {
    ...
    foreach ($platKey in $PlatformKeys) {
        ...
        if ([string]::IsNullOrWhiteSpace($plat.prefix)) {
            Write-Host "    $platKey root files only..." -ForegroundColor DarkGray
            $scaffoldSrc = Join-Path $SysRoot "platforms\$scaffoldPrefix"
            if ($WipeTarget -and $scaffoldPrefix -and (Test-Path (Join-Path $TargetPath $scaffoldPrefix))) {
                Remove-Item (Join-Path $TargetPath $scaffoldPrefix) -Recurse -Force
            }
            $rootPayloadSrc = Join-Path $scaffoldSrc "_rootpayload"
            if ($plat.rootFiles) {
                foreach ($rf in $plat.rootFiles) {
            ...
            Write-Host "  [OK] $($plat.label) (root files only)" -ForegroundColor Green
            continue
        }
...
# ΓöÇΓöÇ Main installer flow ΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇ
Write-Banner
...
# robocopy is far faster than Copy-Item for the ~1450-file framework tree and, with /MIR,
# removes stale embedded files (e.g. orphaned nested scaffolds) that merge-copy leaves behind.
# WipeTarget (clean rebuilds) => /MIR (mirror, deletes stale). Normal updates => /E (additive,
# never deletes user-side framework files, preserving prior merge semantics).
$sysRoboMode = if ($WipeTarget) { "/MIR" } else { "/E" }
robocopy $sysSrc $sysDst $sysRoboMode /NJH /NJS /NFL /NDL /NC /NS /NP /R:1 /W:1 2>$null | Out-Null
if ($LASTEXITCODE -ge 8) { Write-Warning ".gald3r_sys/ robocopy reported errors (code $LASTEXITCODE)" }
...
# Universal files (always installed) ΓÇö deployables from install/project_root when present
$mergeFiles = @{
    ".gitignore"         = "section"
...
# Platform-conditional files ΓÇö sourced from platforms/ subfolder
...
foreach ($file in $mergeFiles.Keys) {
    ...
    # Universal files only in this loop ΓÇö platform-conditional are in $platformFiles loop below
    ...
    switch ($strategy) {
        ...
        "copilot-dir" {
            # Full recursive copy of .github/ ΓÇö only reached when copilot is a selected platform
            if (Test-Path $srcFile) {
                New-Item -ItemType Directory -Force $dstFile | Out-Null
...
# ΓöÇΓöÇ Platform-conditional files (sourced from platforms/ subfolder) ΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇ
foreach ($file in $platformFiles.Keys) {
    $strategy = $platformFiles[$file]
    $srcFile = Join-Path $platformsPayload $file
    $dstFile = Join-Path $TargetPath $file

    # Platform gate ΓÇö skip if platform not selected
    $shouldInstall = switch ($file) {
        "GEMINI.md" {
            $Platforms -contains "gemini"
        }
        "opencode.json" {
        ...
        "AGENTS.md" {
            # Universal ΓÇö any platform install gets AGENTS.md
...
Write-Host ""
Write-Host "  ΓòöΓòÉΓòÉΓòÉΓòÉΓòÉΓòÉΓòÉΓòÉΓòÉΓòÉΓòÉΓòÉΓòÉΓòÉΓòÉΓòÉΓòÉΓòÉΓòÉΓòÉΓòÉΓòÉΓòÉΓòÉΓòÉΓòÉΓòÉΓòÉΓòÉΓòÉΓòÉΓòÉΓòÉΓòÉΓòÉΓòÉΓòÉΓòÉΓòÉΓòÉΓòÉΓòÉΓòÉΓòÉΓòÉΓòÉΓòù" -ForegroundColor Green
Write-Host "  Γòæ            gald3r setup complete!            Γòæ" -ForegroundColor Green
Write-Host "  ΓòÜΓòÉΓòÉΓòÉΓòÉΓòÉΓòÉΓòÉΓòÉΓòÉΓòÉΓòÉΓòÉΓòÉΓòÉΓòÉΓòÉΓòÉΓòÉΓòÉΓòÉΓòÉΓòÉΓòÉΓòÉΓòÉΓòÉΓòÉΓòÉΓòÉΓòÉΓòÉΓòÉΓòÉΓòÉΓòÉΓòÉΓòÉΓòÉΓòÉΓòÉΓòÉΓòÉΓòÉΓòÉΓòÉΓòÉΓò¥" -ForegroundColor Green
Write-Host ""
