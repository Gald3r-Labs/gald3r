...
| `/.claude_cursor_unified_template/` | Root-level template copy (gitignored) |
...
Before `git push`, run **`.cursor/skills/g-skl-git-commit/scripts/gald3r_push_gate.ps1`** or `@g-git-push`:
...
Shared scripts: `.cursor/skills/g-skl-git-commit/scripts/gald3r_push_gate.ps1`; `.cursor/skills/g-skl-git-commit/scripts/gald3r_git_sanity_common.ps1` (secret patterns for `g-hk-pre-commit.ps1`).
