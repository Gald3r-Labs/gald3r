...
& .cursor/skills/g-skl-compliance/scripts/run_compliance_scan.ps1 -Scanner $scanner -RepoRoot (Get-Location).Path
```
...
When called from `.cursor/hooks/g-git-push` or `.cursor/skills/g-skl-compliance/scripts/run_compliance_scan.ps1`:
- Exit 0 → allow push
...
`.cursor/skills/g-skl-compliance/scripts/run_compliance_scan.ps1` — called by SCAN operation and `@g-compliance-gate` hook.
