...
structure (see `.cursor/agents/g-agnt-*.md` and the AGENTS.md
"Agents" roster). Produces:

1. `.cursor/agents/g-agnt-<slug>.md` — full agent contract:
...
| Skill ownership | The new agent MUST reference at least one owned skill (`.cursor/skills/g-skl-*/` entry). An agent with zero skill ownership is overlap with an existing role — either declare ownership or do not hire. |
