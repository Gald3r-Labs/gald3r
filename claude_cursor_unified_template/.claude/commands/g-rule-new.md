...
Creates a new `.claude/rules/g-rl-<NN>-<slug>.md` using the canonical
...
1. Scan `.claude/rules/` to determine the next rule number
2. Collect slug, group, `alwaysApply` (true/false), and `description:` line
3. Write `.claude/rules/g-rl-<NN>-<slug>.md` from the rule template
