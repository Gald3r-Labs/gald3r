...
1. The reviewer asks itself: *"Did this implementation solve a problem type that is NOT covered by
   any existing skill in `.claude/skills/`? Was there a novel, generalizable multi-step
...
auto-promoted to `.claude/skills/` (and therefore never synced to `.cursor/`/`.claude/`). Promotion
