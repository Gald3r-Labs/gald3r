...
Audit the gald3r **system corpus** (`.claude/skills/*/SKILL.md`, `rules/g-rl-*.md`,
...
pwsh -File scripts/gald3r_optimize.ps1 -Mode AUDIT -Path .claude/skills
...
pwsh -File scripts/gald3r_optimize.ps1 -Mode PREVIEW -Path .claude/rules/g-rl-33-enforcement_catchall.md

# APPLY (writes) — ONE file at a time, requires -Confirm, never bulk
pwsh -File scripts/gald3r_optimize.ps1 -Mode APPLY -Path .claude/skills/g-skl-foo/SKILL.md -Confirm
```
