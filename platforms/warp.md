...
- **Adjacent (not equivalent):** **Cloud Triggers & Schedules** on Oz can react to Slack/Linear/GitHub
  ...
  gald3r `.py` scripts on session/tool lifecycle.
...
- **Hooks ❌** — no lifecycle-event system, no `hooks.json`; agent lifecycle hooks are open RFCs
  (#7834 / #6857). Oz Cloud Triggers fire agent runs, not local gald3r `.py` hooks.
...
---
*Reference page — canonical source: `g-skl-platform-warp/PLATFORM_SPEC.md` in the engine repo, generated by `scripts/generate_platform_support_docs.py`. Pages marked *curated* are hand-assessed and not yet live-verified.*
