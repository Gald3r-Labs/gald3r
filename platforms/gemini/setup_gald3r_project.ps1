...
# ──────────────────────────────────────────────────────────────────────────────
# Per-platform structure derived from each platform's PLATFORM_SPEC.md §1 "Folder
# Hierarchy" (the authoritative source of truth), cross-checked against the T1511
# per-platform table. `cats` = universal content subfolders gald3r writes inside the
# platform prefix dir (may be empty @() for platforms with no folder-namespaced tree).
# `rootFiles` = files copied from the platform payload (.gald3r_sys/platforms/<prefix>/)
# to the PROJECT ROOT during install (gracefully skipped when the source is absent).
            ...
            foreach ($rf in $plat.rootFiles) {
                ...
                if (Test-Path $rfSrc) {
                    if ($rf -match "^g-" -or -not (Test-Path $rfDst)) {
                        $rfDstDir = Split-Path $rfDst
                        if ($rfDstDir -and -not (Test-Path $rfDstDir)) { New-Item -ItemType Directory -Force $rfDstDir | Out-Null }
                        Copy-Item $rfSrc $rfDst -Recurse -Force
                        if (-not $Quiet) { Write-Host "  $platKey root file: $rf" }
                    }
                }
            }
        }
            ...
            foreach ($rf in $plat.rootFiles) {
                ...
                if (Test-Path $rfSrc) {
                    if ($rf -match "^g-" -or -not (Test-Path $rfDst)) {
                        ...
                        if ($rfDstDir -and -not (Test-Path $rfDstDir)) { New-Item -ItemType Directory -Force $rfDstDir | Out-Null }
                        Copy-Item $rfSrc $rfDst -Recurse -Force
                        Write-Host "    $platKey root file: $rf" -ForegroundColor DarkGray
                    }
                }
            }
        }
