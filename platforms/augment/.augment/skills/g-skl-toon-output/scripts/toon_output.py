...
# [ 	]* not \s* (BUG-486/T510 class) -- inputs are split lines today, but
# a two-line string would let \s* cross the newline and mis-capture.
_SCALAR_ARR_RX = re.compile(r"^([\w-]+)\[(\d+)\]:[ \t]*(.*)$")
_KEY_VAL_RX = re.compile(r"^([\w-]+):[ \t]*(.*)$")
...
def find_project_root(*, ambiguous_candidates: "list[Path] | None" = None) -> str:
    ...
    T516 (T512 inventory row 25 -- low blast radius: report output path): a
