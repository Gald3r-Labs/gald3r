...
argument-hint: '[tasks <id...>] [bugs <BUG-id...>] [--provider <id>[:<model>]] [--model <id>] [--reviewer-provider <id>] [--reviewer-model <id>]'
...
> `gald3r <verb>` call this coordinator or any reviewer bucket makes MUST run as `uv run gald3r
> <verb>`, never bare `gald3r` — a stale PATH binary can silently shadow this checkout's dev
> source, including a reviewer's own PASS/FAIL status-write landing in the wrong `.gald3r/` across
> a worktree boundary. See `g-go.md` Step 0b for full text and the optional staleness hard-fail
> check.
...
All arguments pass through, including T580's `--provider`/`--model`/`--reviewer-provider`/
`--reviewer-model` (BUG-612 companion) — each round-robin reviewer bucket resolves its own
target independently (see `@g-go-review`'s "Provider & Model Selection" section); a Cursor host
maps to `cursor-agent` + `gpt-5.6-terra-medium` by default unless overridden.
