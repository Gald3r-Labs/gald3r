...
from stdin, runs the event's concern chain, merges any `additional_context`,
and emits a single `{ "continue": true }` envelope.

## Side Effects

- None of its own. Side effects come only from concern hooks registered in
  `g_hk_core.CONCERN_CHAIN["tool-end"]` — as of T90, `g-hk-tel-tap --kind
  post_tool_use` (maps the PostToolUse payload onto the TEL normalized
  stream; SILENT, contributes no `additional_context`, never blocks).
