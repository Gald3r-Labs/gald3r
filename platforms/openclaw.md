...
handlers are **TypeScript** (`handler.ts`) with an OpenClaw-specific event taxonomy, so gald3r
`.py` hooks are not directly portable; (2) rules are **prose** in `AGENTS.md` / `SOUL.md` (no `.mdc` typed
...
**hooks** (rewrite `.py` → `handler.ts`) and pointing the workspace at the gald3r skills source.
...
- Hooks require a **TypeScript rewrite** (`handler.ts`) — do not assume `.py` portability. Treat the
...
> `.py`→`handler.ts` hook wiring itself remains install-unverified.
...
| Caveats | Hook handlers TypeScript-only (`handler.ts`); rules prose-only (no `.mdc`); skills install to `~/.openclaw/workspace/skills/`; gald3r `.py` hook wiring [STUB] / unverified |


---
*Reference page — canonical source: `g-skl-platform-openclaw/PLATFORM_SPEC.md` in the engine repo, generated by `scripts/generate_platform_support_docs.py`. Pages marked *curated* are hand-assessed and not yet live-verified.*
