...
| 9 | **Parity pipeline does NOT target `.vibe/`** — neither `gald3r sync` nor `gald3r platform install` has a `.vibe/` writer (and the historical `platform_parity_sync.ps1` never existed on disk, C-NO-PS1/T628). Any real install support is future work. | Medium <!-- ps1-gate-allow --> |
...
| No `.vibe/` writer in parity pipeline | Verified absence — `gald3r sync` / `gald3r platform install` target `.cursor/`/`.claude/` family, not `.vibe/` |


---
*Reference page — canonical source: `g-skl-platform-mistral/PLATFORM_SPEC.md` in the engine repo, generated by `scripts/generate_platform_support_docs.py`. Pages marked *curated* are hand-assessed and not yet live-verified.*
